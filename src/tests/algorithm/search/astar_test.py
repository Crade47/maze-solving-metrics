...
def test_astar_simple_path():
    """
    Test AStar on a simple 2x2 maze with a clear path:
...
def test_astar_no_path():
    """Test AStar when there is no valid path to the target (all walls intact)."""
    maze = Maze(2)
...
def test_astar_multiple_paths():
    """
    Test AStar with multiple possible paths to target:
    Path 1: (0,0) -> (0,1) -> (1,1)
    Path 2: (0,0) -> (1,0) -> (1,1)
    """
    maze = Maze(2)
...
def test_astar_larger_maze():
    """
    Test AStar on a 3x3 maze with a winding path:
...
def test_astar_single_cell():
    """Test AStar on a 1x1 maze (edge case)."""
    maze = Maze(1)
    solver = AStar(maze)
    path = solver.solve()
    expected_path = [(0, 0)]
