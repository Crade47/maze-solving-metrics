...
def test_bfs_simple_path():
    """
    Test BFS on a simple 2x2 maze with a clear path:
...
def test_bfs_no_path():
    """Test BFS when there is no valid path to the target (all walls intact)."""
    maze = Maze(2)
...
def test_bfs_multiple_paths():
    """
    Test BFS with multiple possible paths to target:
    Path 1: (0,0) -> (0,1) -> (1,1)
    Path 2: (0,0) -> (1,0) -> (1,1)
    """
    maze = Maze(2)
...
def test_bfs_larger_maze():
    """
    Test BFS on a 3x3 maze with a winding path:
...
def test_bfs_single_cell():
    """Test BFS on a 1x1 maze (edge case)."""
    maze = Maze(1)
    solver = BFS(maze)
    path = solver.solve()
    expected_path = [(0, 0)]
